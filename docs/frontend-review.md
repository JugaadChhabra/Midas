...
- [x] #6 Resolve tabbed-SPA vs separate-page split for Performance — done (user chose fold). Added a "Performance" channel tab that embeds /performance via a same-origin iframe (embed=1 hides that page's nav/breadcrumb/title; iframe auto-sizes to content via ResizeObserver). Chose the iframe over a literal 500-line JS/CSS merge to avoid name collisions (renderKpis, load, sync-btn, toasts, …) I couldn't runtime-verify. "View performance →" now switches to the tab; the standalone /performance page still works for direct links.
...
- [x] #20 Factor shared time/number formatting helpers — done fixed the concrete number-format inconsistencies (index applied_7d_total and channel Overview pending/applied counts now use fmt() like their neighbors). DEFERRED the actual extraction into a shared format.js: the pages use classic (non-module) <script>, so moving fmt/escapeHtml/timeAgo into a shared file means removing per-page top-level `const` definitions to avoid cross-script redeclaration errors — a wide change I can't runtime-verify blind (node --check only sees one script at a time). Recommend doing it alongside the existing swr.js include, with manual page testing.
