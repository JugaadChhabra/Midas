...
def all_rows(query, page_size: int = PAGE_SIZE) -> list:
...
def all_rows_parallel(build_query: Callable[..., object], *,
                      page_size: int = PAGE_SIZE, max_workers: int = 5) -> list:
    ...
    first = build_query(count="exact").range(0, page_size - 1).execute()
    ...
    def _page(off: int) -> list:
        return build_query().range(off, off + page_size - 1).execute().data or []
...
def rows_for_ids(build_query: Callable[[list], object], ids, chunk: int = IN_CHUNK) -> list:
    ...
    for i in range(0, len(ids), chunk):
        out.extend(all_rows(build_query(ids[i:i + chunk])))
    return out
